...
export default function ProductInventoryPage() {
  ...
  const fetchStock = async () => {
    const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/inventory/stock/${productId}`);
    setStock(await res.json());
  };

  const fetchMovements = async () => {
    const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/inventory/movements?productId=${productId}&limit=20`);

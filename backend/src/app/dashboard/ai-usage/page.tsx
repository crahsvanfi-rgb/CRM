...
export default function AiUsagePage() {
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      const summaryRes = await fetch(`/api/ai-usage/summary?fechaInicio=${dateRange.inicio}&fechaFin=${dateRange.fin}`);
      if (!summaryRes.ok) throw new Error('Error al cargar el resumen');
      const summaryData = await summaryRes.json();
      setSummary(summaryData);

      const logsRes = await fetch(`/api/ai-usage?fechaInicio=${dateRange.inicio}&fechaFin=${dateRange.fin}&limit=50`);

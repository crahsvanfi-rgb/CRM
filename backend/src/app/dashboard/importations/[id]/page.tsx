...
export default function ImportationDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const fetchImportation = async () => {
    try {
      const res = await fetch(`/api/importations/${id}`, {
  ...
  const handleTransition = async (newState: string) => {
    ...
    try {
      const res = await fetch(`/api/importations/${id}/transition`, {
        method: 'POST',
        headers: {
  ...
  const handleReceive = async () => {
    ...
    try {
      const res = await fetch(`/api/importations/${id}/receive`, {
        method: 'POST',
        headers: {
          'x-tenant-id': 'test-tenant',
  ...
  const handleClose = async () => {
    ...
    try {
      const res = await fetch(`/api/importations/${id}/close`, {
        method: 'POST',
        headers: { 'x-tenant-id': 'test-tenant' }
      });
  ...
  const generatePDF = () => {
    window.open(`/api/importations/${id}/pdf`, '_blank');
  };

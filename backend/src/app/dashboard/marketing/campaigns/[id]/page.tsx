...
export default function CampaignDetailsPage() {
  ...
  const handleSaveVelocity = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch(`/api/campaigns/${id}/velocity`, {
        method: 'PATCH',
  ...
  const fetchCosts = async () => {
    try {
      const res = await fetch(`/api/campaigns/${id}/costs`);
      if (res.ok) {
        const data = await res.json();
        setCostsData(data);
      }
  ...
  const handleCalculateCosts = async () => {
    ...
    try {
      const res = await fetch(`/api/campaigns/${id}/costs/calculate`, { method: 'POST' });
      if (res.ok) {

...
export default function SuppliersPage() {
  ...
  const handleSave = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      const url = editingId ? `${apiUrl}/suppliers/${editingId}` : `${apiUrl}/suppliers`;
      ...
      console.error(error);
      alert('Error al guardar: ' + (error.message || 'No se pudo conectar'));
    }
  };
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      ...
      console.error(error);
      alert('Error al eliminar: ' + (error.message || 'Error de red'));
    }
  };

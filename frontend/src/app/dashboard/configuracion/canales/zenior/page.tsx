...
export default function ZeniorConfigPage() {
  ...
  const fetchConfig = useCallback(async () => {
    try {
      ...
      const res = await fetch(apiPath('/external-channels'), {
        headers: {
  ...
  const fetchConversations = useCallback(async () => {
    try {
      const token = localStorage.getItem('supabase_token') || '';
      const tenantId = localStorage.getItem('tenant_id') || '';
      const res = await fetch(apiPath('/external-channels/conversations'), {
        headers: {
          'Authorization': `Bearer ${token}`,
          'x-tenant-id': tenantId
        }
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = await fetch(apiPath('/external-channels/zenior'), {
        method: 'PUT',
        headers: {
  ...
  const handleTestConnection = async () => {
    try {
      ...
      const token = localStorage.getItem('supabase_token') || '';
      const tenantId = localStorage.getItem('tenant_id') || '';
      const res = await fetch(apiPath('/external-channels/zenior/test'), {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${token}`,
          'x-tenant-id': tenantId
      ...
      setTestResult({ status: 'ERROR', message: 'Error de red' });
    }
  };
  ...
  const viewMessages = async (id: string) => {
    ...
    try {
      const token = localStorage.getItem('supabase_token') || '';
      const tenantId = localStorage.getItem('tenant_id') || '';
      const res = await fetch(apiPath(`/external-channels/conversations/${id}/messages`), {
        headers: {
          'Authorization': `Bearer ${token}`,
          'x-tenant-id': tenantId
        }

...
export default function CampaignConfigPage() {
  ...
  const fetchConfig = async () => {
    try {
      setLoading(true);
      const res = await fetch(apiPath('/campaign-config'), {
        headers: {
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch(apiPath('/campaign-config'), {
        ...
        body: JSON.stringify((() => {
          const { id, tenantId, createdAt, updatedAt, estado, ...clean } = config as any;
          return {
            ...clean,
          ...config,
  ...
  const handleToggle = async () => {
    setToggling(true);
    try {
      const res = await fetch(apiPath('/campaign-config/toggle'), {
        method: 'PATCH',
        headers: {
          'Authorization': `Bearer ${typeof window !== 'undefined' ? localStorage.getItem('token') || '' : ''}`
        }

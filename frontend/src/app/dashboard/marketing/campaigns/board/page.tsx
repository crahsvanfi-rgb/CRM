...
export default function CampaignsBoardPage() {
  ...
  const fetchCampaigns = async () => {
    try {
      ...
      const res = await fetch(apiPath(`/campaigns/board?${params.toString()}`), {
  ...
  const loadProducts = async () => {
    try {
      const res = await fetch(apiPath('/products?limit=100'));
      if (res.ok) {
  ...
  const openSimulateModal = async (campaign: any) => {
    ...
    try {
      const res = await fetch(apiPath(`/campaigns/${campaign.id}/recipients`));
      if (res.ok) {
        const data = await res.json();
  ...
  const handleCreateRecover = async (e: React.FormEvent) => {
    ...
    setSubmitting(true);
    try {
      const res = await fetch(apiPath('/campaigns/auto/recover-inactive'), {
        method: 'POST',
  ...
  const handleCreateProduct = async (e: React.FormEvent) => {
    ...
    setSubmitting(true);
    try {
      const res = await fetch(apiPath('/campaigns/auto/product'), {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  const handleCreateVendor = async (e: React.FormEvent) => {
    ...
    setSubmitting(true);
    try {
      const res = await fetch(apiPath('/campaigns/auto/vendor'), {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  const handleProcessResponse = async (e: React.FormEvent) => {
    ...
    setSubmitting(true);
    try {
      const res = await fetch(apiPath(`/campaigns/${selectedCampaignForSim.id}/process-response`), {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },

...
export default function AiAgentPage() {
  ...
  const fetchConversations = useCallback(async () => {
    try {
      ...
      if (!res.ok) {
        res = await fetch(apiPath('/ai-chat/conversations'), { headers });
      }
  ...
  const fetchMessages = useCallback(async (convId: string) => {
    try {
      ...
      if (!res.ok) {
        res = await fetch(apiPath(`/ai-chat/conversations/${convId}`), { headers });
      }
  ...
  const handleNewConversation = async (initialTitle?: string): Promise<string | null> => {
    ...
    try {
      ...
      if (!res.ok) {
        res = await fetch(apiPath('/ai-chat/conversations'), {
          method: 'POST',
          headers,
          body: JSON.stringify({ titulo: title }),
        });
  ...
  const handleSendMessage = async (textToSend?: string) => {
    ...
    try {
      ...
      if (!res.ok) {
        res = await fetch(apiPath(`/ai-chat/conversations/${activeId}/messages`), {
          method: 'POST',
          headers,
          body: JSON.stringify({ contenido: text }),
        });
